...
config = Config(dataset_name='lastfm')
...
def run_main(config_wb=None):
  ...
  wandb.init(project="lastfm", config=config_wb, name=config_wb.get('name'))
  ...
  if config.dataset_name == 'lastfm':
    svdg = SmoothStart(list(config.ui_dims), list(config.user_dims), [config_t.nbins, config_t.nbins_i],\
                           ...
                           dataset=None, dataset_name=config_t.dataset, dropout=dropout).to(device)
  ...
  # global variable passing through config
  ...
  if config_t.dataset == 'movielens':
    n_epoch, stop_rounds = 120, 8
  else:
    n_epoch, stop_rounds = 3, 2
...
config.counts_tr = cold_dataset.counts_tr
...
config.dic['path'] = './results/last_dcn_v2_exp_ui'

config_wb = {
        'name':'last_dcn_v2_exp_ui',
        'lr':1e-3, 
        'wd':0,
        'dropout':0,
        'lr_f':None,
...
# control, no ui
config.dic['path'] = './results/last_dcn_v2_control_no_ui'

bounds = config.bounds

config_wb = {
        'name':'last_dcn_v2_control_no_ui',
        'lr':1e-3, 
        'wd':0,
        'dropout':0,
        'lr_f':None,
